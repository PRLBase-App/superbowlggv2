  ...
  return (
    <div className="flex items-center gap-3 pr-3" aria-hidden={duplicate || undefined}>
      {teams.map((team) => duplicate ? (
        <div key={`duplicate-${team.id}`} className="logo-tile flex h-16 w-24 shrink-0 items-center justify-center rounded-2xl border border-brand-border p-3 shadow-sm">
        ...
        <Link key={team.id} href={`/nfl/teams/${team.slug}`} className="logo-tile flex h-16 w-24 shrink-0 items-center justify-center rounded-2xl border border-brand-border p-3 shadow-sm transition hover:-translate-y-1 hover:border-brand-primary/40" aria-label={`Open ${team.name} team page`}>
