...
export function VerificationComplete({ referralCode, verificationError }: { referralCode?: string; verificationError?: string }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [referralCode, router, verificationError]);
    ...
    <div className="mx-auto w-full max-w-md">
      <div className="card text-center">
        ...
        <div className="mt-6 flex justify-center gap-2">
          {state === "ready" ? <Link href="/" className="btn-primary">Continue</Link> : null}
          {state === "error" ? <Link href="/auth/sign-in" className="btn-secondary">Log in</Link> : null}
        </div>
      </div>
    </div>
  );
