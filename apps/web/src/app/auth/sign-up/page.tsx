...
export default async function SignUpPage({ searchParams }: { searchParams: Promise<{ ref?: string; next?: string }> }) {
  ...
  return <AuthForm mode="sign-up" returnTo={safeReturnTo(next, "/")} initialRef={ref} googleEnabled={Boolean(configuration.AUTH_GOOGLE_ID && configuration.AUTH_GOOGLE_SECRET)} telegramBotUsername={configuration.TELEGRAM_BOT_USERNAME} />;
}
